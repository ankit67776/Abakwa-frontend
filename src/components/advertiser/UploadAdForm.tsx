...
const UploadAdForm: React.FC<UploadAdFormProps> = ({ onSubmit }) => {
  ...
  const getFileIcon = (file: File) => {
    ...
    if (type === 'text/html' || type === 'application/javascript' || type.includes('zip')) return <FileText className="h-5 w-5 text-orange-500" />;
        ...
        <CardContent className="p-6 sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-12">
            ...
            <section>
              {renderSectionTitle("Creative Files", "Upload your image, video, or HTML5 ad files.")}
               <div
                ...
                className={cn(
                  ...
                  isDragging ? 'border-primary bg-primary/5' : 'border-input hover:border-primary/70 bg-secondary/20',
              ...
              {files.length > 0 && (
                <div className="mt-6 space-y-3 max-h-72 overflow-y-auto pr-2 rounded-lg border p-4 bg-secondary/30">
                  {files.map((file, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-card rounded-lg border shadow-sm hover:shadow-md transition-shadow">
            ...
            <Accordion type="multiple" className="w-full space-y-6" defaultValue={["advanced-settings"]}>
              {[
                  ...
                  <AccordionTrigger className="px-6 py-4 text-lg font-medium hover:no-underline hover:bg-secondary/50 transition-colors data-[state=open]:bg-secondary/50 data-[state=open]:border-b">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                       ...
                       <ChevronDown className="h-5 w-5 shrink-0 transition-transform duration-200 text-primary group-data-[state=open]:rotate-180 ml-auto" />
                    </div>
                  </AccordionTrigger>
                  <AccordionContent className="px-6 py-6 space-y-6 bg-background/50 border-t">
                    {item.id === "advanced-settings" && (
                      <>
                        ...
                        {headerBidding && (
                          <FormFieldContainer className="pl-12 mt-4">
            ...
            <CardFooter className="pt-10 pb-0 px-0 flex flex-col sm:flex-row sm:justify-end space-y-3 sm:space-y-0 sm:space-x-3 border-t mt-12">
              <Button
                type="button"
                variant="outline"
