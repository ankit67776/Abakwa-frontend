...
interface ViewsByPageTableProps {
  title: string;
  data: {
    headers: string[];
    rows: (string | number)[][];
...
const ViewsByPageTable: React.FC<ViewsByPageTableProps> = ({ title, data }) => {
    ...
    <Card className="shadow-sm hover:shadow-md transition-shadow h-full flex flex-col">
              ...
              {displayRows.map((row, rowIndex) => (
                <TableRow key={rowIndex} className="text-xs hover:bg-muted/30">
                  {row.map((cell, cellIndex) => (
                    <TableCell 
                      key={cellIndex} 
                      className={`px-2 py-1.5 whitespace-nowrap truncate max-w-[150px] sm:max-w-[200px] ${data.columnAlignments?.[cellIndex] === 'right' ? 'text-right font-medium text-foreground' : 'text-left text-muted-foreground'}`}
      ...
      <div className="px-4 pb-3 mt-auto border-t pt-3">
        <Button variant="link" size="sm" className="text-xs text-primary p-0 h-auto hover:underline">
          View pages and screens <ArrowRight className="ml-1 h-3 w-3" />
        </Button>
      </div>
    </Card>
  );
...
export default ViewsByPageTable;
