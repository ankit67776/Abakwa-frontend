...
const AdBrowser: React.FC<AdBrowserProps> = ({ ads, onSelectAd }) => {
  ...
  const filteredAds = ads.filter((ad) => {
    ...
    // if (selectedCategory) {
    //   matches = matches && ad.category === selectedCategory; // Assuming ad has a category field
    // }
      ...
      <Card>
        ...
        <CardContent className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 md:flex-shrink-0">
            <Select value={selectedFormat || ''} onValueChange={(value) => setSelectedFormat(value === '' ? null : value as Ad['format'])}>
              ...
              <SelectContent>
                ...
                {formats.map((format) => (
                  <SelectItem key={format.id} value={format.id!}> {/* Added ! for potentially undefined id */}
                    {format.name}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          {filteredAds.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <Card key={ad.id} className="overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <Image
                        src={ad.imageUrl || ad.thumbnailUrl || "https://placehold.co/600x338.png?text=Ad+Preview"}
                        alt={ad.name}
                        fill
