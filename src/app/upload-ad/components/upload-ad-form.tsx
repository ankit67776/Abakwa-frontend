...
import { Upload, X, Plus, Image as ImageIcon, FileText, Film, Calendar, DollarSign, Target, Info, Loader2 } from 'lucide-react';
...
const UploadAdForm: React.FC<UploadAdFormProps> = ({ onSubmit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (files.length === 0 && adFormat !== 'text') { // Text ads might not need a file
      alert('Please upload at least one file for non-text ads.');
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <CardFooter className="pt-8 pb-0 px-0">
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full md:w-auto ml-auto bg-accent text-accent-foreground hover:bg-accent/90"
              size="lg"
            >
              {isLoading ? (
